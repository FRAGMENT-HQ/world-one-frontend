const ColorAdjustor = ({ onClick , unsplash3PyBkxgTiL0, welcomeToWorldOneForexBlo }) => {
  return (
    <div className=" min-w-0 w-[100%] sm:w-[30%] sm:min-w-[360px] rounded-13xl bg-darkslateblue-500 shadow-[0px_6px_24px_-4px_rgba(18,_25,_56,_0.1),_0px_12px_48px_4px_rgba(18,_24,_56,_0.15)] [backdrop-filter:blur(48px)] overflow-hidden shrink-0 flex flex-col items-start justify-start py-9 px-4 tablet:px-8 box-border relative gap-[48px] text-left text-xl text-text5 font-body-small mq825:pt-[23px] mq825:pb-[23px] mq825:box-border">
      ...
      <button onClick={onClick} className="cursor-pointer [border:none] py-3 px-5 bg-white self-stretch rounded-2xl overflow-hidden flex flex-row items-center justify-center gap-[6px]">
        <div className="flex flex-col items-center justify-center pt-2 px-0 pb-0">
          <div className="relative text-3xl  font-semibold font-lato text-text1 text-left mq450:text-3xl mq450:leading-[26px]">
            Read More
          </div>
        </div>
        <img
